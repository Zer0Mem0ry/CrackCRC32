...
unsigned int CRC32(const char *pData, size_t iLen)
{
...
string StrCRC32(string input)
{
	std::stringstream stream;
	stream << std::hex << CRC32((char*)input.c_str(), input.length());
...
void Generate(string source, unsigned int hash, string seed, size_t length)
{
	static const double startTick = GetTickCount();
	...
	{
		auto position = find(Hashes.begin(), Hashes.end(), CRC32(seed.c_str(), seed.length()));

		if (position != Hashes.end())
		{
			Hashes.push_back(CRC32(seed.c_str(), seed.length()));
			...
			cout << "Collision #" << collisions
				<< ", Source: " << seed << ", Duplicate: " << Sources[distance(Hashes.begin(), position)]
				<< ", Hash: 0x" << StrCRC32(seed) << ", Iterations: " << iterations << endl
				<< "Ticks: " << endTick << ", Seconds: " << endTick / 1000 << ", Minutes: "
				<< endTick / 1000 / 60 << ", Hours: " << endTick / 1000 / 60 / 60;

			cout << endl << endl;

			SetConsoleTitle(to_string(collisions).c_str());
		}
		else {
			Hashes.push_back(CRC32(seed.c_str(), seed.length()));
			Sources.push_back(seed);
	...
	for (unsigned int i = 0; i < source.length(); i++) // iterate through alphabet
	{
		string appended = seed + source[i];

		Generate(source, hash, appended, length - 1);
	}
}
*/